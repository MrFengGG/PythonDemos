...
def login(url, username, password):
    ...
    submit = browser.find_element_by_xpath(
        '//*[@id="pl_login_form"]/div/div[3]/div[6]/a/span')  # 找到用于提交表单的类型为submit的input
    submit.click()  # 点击
    time.sleep(5)
    WebDriverWait(browser, 10).until(
        expected_conditions.presence_of_element_located((By.CLASS_NAME, 'WB_miniblog')))  # 等待登陆页面加载完成
...
def getweibo(filename, html):
    ...
    for weibo in weibos:
        ...
        if len(weibopics) != 0:  # 判断标签是否存在
            ...
            for pic in pics:
                download(pic["src"], filename + "\\" + str(pic_num + 1)+".jpg")
                pic_num += 1
                file.write(pic["src"] + "\n")
